...
def op2_gen_seq(master, date, consts, kernels, suffix='', user_kernels=[]):
...
##########################################################################
    ...
    if len(user_kernels)==0:
      if FORTRAN:
        code('include '+name+'.inc')
      elif CPP:
        code('#include "../'+decl_filepath+'"')
    else:
      code('#include "../'+user_kernels[nk]+'"')
...
##########################################################################
    ...
    code('void op_par_loop_'+name+suffix+'(char const *name, op_set set,')
    depth += 2
...
##########################################################################
    if not os.path.exists('seq'):
        os.makedirs('seq')
    fid = open('seq/'+name+'_seqkernel'+suffix+'.cpp','w')
...
##########################################################################
  ...
  for nk in range(0,len(kernels)):
    code('#include "'+kernels[nk]['name']+'_seqkernel'+suffix+'.cpp"')
  master = master.split('.')[0]
  fid = open('seq/'+master.split('.')[0]+'_seqkernels'+suffix+'.cpp','w')
  fid.write('//\n// auto-generated by op2.py\n//\n\n')
  fid.write(file_text)
  fid.close()
