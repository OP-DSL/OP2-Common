...
<<<<<<< HEAD:translator/c/python/op2_gen_cuda_simple.py
          code('<ARG>.data = (char *)<ARG>h;')
          code('op_mpi_reduce(&<ARG>,<ARG>h);')
=======
          code('ARG.data = (char *)ARGh;')
          code('op_mpi_reduce(&ARG,ARGh);')
>>>>>>> Restructuring codegenerators with new directories seperating them out for just in time (jit) and ahead of time (aot) compilation:translator/c/python/aot/op2_gen_cuda_simple.py
