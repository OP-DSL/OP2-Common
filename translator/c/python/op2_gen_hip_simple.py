...
def op2_gen_hip_simple(master, date, consts, kernels,sets, macro_defs):
...
##########################################################################
#  start with HIP kernel function
##########################################################################
    ...
    for nc in range(0,len(consts)):
      ...
      body_text = re.sub('\\b'+varname+'\\b', varname+'_hip',body_text)

    signature_text = '__device__ '+head_text + '_gpu( '+signature_text + ') {'
    file_text += signature_text + body_text + '}\n'

    comm('')
    comm(' HIP kernel function')

    if FORTRAN:
      code('subroutine op_hip_'+name+'(')
    elif CPP:
      code('__global__ void op_hip_'+name+'(')
...
#
    if ninds>0 and not op_color2 and not atomics:
      ...
      code('')
      code('int blockId = blkmap[hipBlockIdx_x + hipBlockIdx_y*hipGridDim_x  + block_offset];')
      code('')
      ...
      if ind_inc:
        code('nelems2  = hipBlockDim_x*(1+(nelem-1)/hipBlockDim_x);')
      ...
      if ind_inc:
        FOR_INC('n','hipThreadIdx_x','nelems2','hipBlockDim_x')
        code('int col2 = -1;')
        k = []
        for g_m in range(0,nargs):
        ...
        FOR_INC('n','hipThreadIdx_x','nelem','hipBlockDim_x')
        k = []
        for g_m in range(0,nargs):
          if maps[g_m] == OP_MAP and (not mapinds[g_m] in k):
            k = k + [mapinds[g_m]]
...
#
      ...
      code('int tid = hipThreadIdx_x + hipBlockIdx_x * hipBlockDim_x;')
      IF('tid + start < end')
      if atomics:
...
#
      ...
      FOR_INC('n','hipThreadIdx_x+hipBlockIdx_x*hipBlockDim_x','set_size','hipBlockDim_x*hipGridDim_x')
    ...
    if reduct:
       ...
       for m in range (0,nargs):
         ...
         if maps[m]==OP_GBL and accs[m]!=OP_READ and accs[m] != OP_WRITE:
           FOR('d','0','<DIM>')
           if accs[m]==OP_INC:
             code('op_reduction<OP_INC>(&<ARG>[d+hipBlockIdx_x*<DIM>],<ARG>_l[d]);')
           elif accs[m]==OP_MIN:
             code('op_reduction<OP_MIN>(&<ARG>[d+hipBlockIdx_x*<DIM>],<ARG>_l[d]);')
           elif accs[m]==OP_MAX:
             code('op_reduction<OP_MAX>(&<ARG>[d+hipBlockIdx_x*<DIM>],<ARG>_l[d]);')
           else:
...
#
    ...
    if ninds == 0 or atomics:
      comm('set HIP execution parameters')
...
#
    if ninds>0 and not atomics:
      ...
      if reduct or (inc_stage==1 and ind_inc):
        ...
        code('hipLaunchKernelGGL(op_hip_'+name+',nblocks,nthread,nshared,0,') #code('op_hip_'+name+'<<<nblocks,nthread,nshared>>>(')
      else:
        code('hipLaunchKernelGGL(op_hip_'+name+',nblocks,nthread,0,0,') #code('op_hip_'+name+'<<<nblocks,nthread>>>(')
...
#
    ...
    code('hipSafeCall(hipDeviceSynchronize());')
...
##########################################################################
    if not os.path.exists('hip'):
        os.makedirs('hip')
    fid = open('hip/'+name+'_kernel.cpp','w')
...
##########################################################################
  ...
  code('#include "op_hip_rt_support.h"')
  code('#include "op_hip_reduction.h"')
  ...
  for nc in range (0,len(consts)):
    if consts[nc]['dim']==1:
      code('__constant__ '+consts[nc]['type'][1:-1]+' '+consts[nc]['name']+'_hip;')
    else:
  ...
  for nc in range(0,len(consts)):
    ...
    if not consts[nc]['dim'] or int(consts[nc]['dim']) > 1:
      ...
      ENDIF()
    code('hipSafeCall(hipMemcpyToSymbol(HIP_SYMBOL('+consts[nc]['name']+'_hip), dat, dim*size));')
    ENDIF()
    code('else ')
  ...
  fid = open('hip/'+master.split('.')[0]+'_kernels.cpp','w')
  fid.write('//\n// auto-generated by op2.py\n//\n\n')
  fid.write(file_text)
  fid.close()
