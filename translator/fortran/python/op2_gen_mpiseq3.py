...
##########################################################################
    ...
    if ind_inc and repro_loop and repr_temp_array:
      ...
      for i in range(0,ninds):
        if maps[invinds[i]] == OP_MAP and accs[invinds[i]] == OP_INC:
          code('call c_f_pointer(op_get_reproducible_tmparray(opArg'+str(invinds[i]+1)+',repr_from_size, repr_to_size),opDat'+str(invinds[i]+1)+'TmpPtr, (/n_upper * getMapDimFromOpArg(opArg'+str(invinds[i]+1)+') * opArg'+str(invinds[i]+1)+'%dim/))')
