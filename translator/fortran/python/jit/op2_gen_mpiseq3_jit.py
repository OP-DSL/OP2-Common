...
##########################################################################
  ...
  specified_kernels = 'IFLUX_EDGEK', 'VFLUX_EDGEK', 'SRCSA_NODEK', 'UPDATEK', 'JC27HIS_PRESSURE', \
  'ACCUMEDGES', 'GRADL_EDGECONK', 'PERIODICITYK', 'GRADL_DIVIDEK'
  ...
  'INITCL_CLINEK',\
  'SET_SHROUD_QB_BND', \
  'SET_SHROUD_SET_QB_BND', \
  'SET_MASS_QB_BND', \
  'SET_MASS_SET_QB_BND', \
  'SHRDLK_MASS_BND', \
  'SET_TULE_QB_BND_KE', \
  'SET_TULE_QB_BND_KO', \
  'SET_INLET_NORMAL_QB_BND', \
  'SET_WALLTU_QB_INIT', \
  'SET_WALLTU_QB_COPY', \
  'SET_WALLTU_SET_QB_BND'
  '''
...
##########################################################################
    ...
    code('')
    code('IMPLICIT NONE')
    code('character(kind=c_char,len='+str(len(name))+'), INTENT(IN) :: userSubroutine')
...
##########################################################################
    ...
    fid.write(header+ user_kernel + wrapper + sig + \
...
##########################################################################
  ...
  code('call execute_command_line ("make -j JIT_seqkernel_rec.so", exitstat=STATUS)')
  ENDIF()
