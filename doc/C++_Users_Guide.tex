...
{This routine transfers a copy of the data currently held in an op\_dat from the OP2 backend to a user allocated memory
block.}
...
{This routine (which is not yet implemented) defines a temporary dataset, initialises it to zero, and returns a dataset ID.}
...
{This routine (which is not yet implemented) terminates a temporary dataset.}
...
{This routine simply prints a message; this routine is created is in place of the standard {\tt printf}
...
{This routine transfers a copy of the op\_dat's data currently held by OP2 to a user allocated block of memory pointed
to by data pointer of type T. the low and high integers gives the range of elements to be fetched}
\item [dat] OP dataset ID -- The op dat who's data is to be fetched from OP2 space to user space

\item [data] pointer to a block of memory of type T -- allocated by the user
\item [low] index of the 1st element to be fetched
...
\begin{routine} {void op\_dump\_hdf5(char const * file\_name)}
