...
supporting functions and data migration routines to facilitate the above goals.\\
\indent OP2 partitions the mesh immediately after all the calls to \texttt{op\_decl\_*}. For this OP2 assumes that an
...
in \texttt{op\_mpi\_part\_core.c}. A wrapper function is required to organize the data and/or mesh elements into a
format that is acceptable to the ParMetis and PT-Scotch partitioning routines. We anticipate that supporting other
different partitioners may require new wrapper functions to be developed into the MPI back-end.
