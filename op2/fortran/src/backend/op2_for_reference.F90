...
#define ARGT(x) type(op_dat_core) :: data##x @ integer(kind=c_int), value :: itemSel##x, access##x @ type(op_map_core) :: map##x @
...
#define ARG_NOCORE(x) type(op_dat) :: data##x @ integer(kind=c_int), value :: itemSel##x, access##x @ type(op_map) :: map##x @
