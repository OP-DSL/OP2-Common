...
// user function
__device__ void res_gpu(const float *A, const float *u, float *du,
                        const float *beta) {
  *du += (*beta) * (*A) * (*u);
}
...
op_cuda_res(const float *__restrict ind_arg0, float *__restrict ind_arg1,
            const int *__restrict opDat1Map, const float *__restrict arg0,
  ...
  for (int n = threadIdx.x; n < nelems2; n += blockDim.x) {
    ...
    if (n < nelem) {
      ...
      res_gpu(arg0 + (n + offset_b) * 1, ind_arg0 + map1idx * 1, arg2_l, arg3);
    ...
    for (int col = 0; col < Plan->ncolors; col++) {
      ...
      if (Plan->ncolblk[col] > 0) {
        ...
        op_cuda_res<<<nblocks, nthread, nshared>>>(
            ...
            (float *)arg0.data_d, (float *)arg3.data_d, Plan->ind_map,
