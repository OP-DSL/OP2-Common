...
void op_par_loop_update(char const *name, op_set set,
  ...
  if (set->size >0) {
    ...
    reduct_size   = MAX(reduct_size,sizeof(double)*64);
    ...
    for (int b=0; b<maxblocks; b++)
      for (int d=0; d<1; d++)
        ((double *)arg4.data)[d+b*1*64] = ZERO_double;
    reduct_bytes += ROUND_UP(maxblocks*1*sizeof(double)*64);
...
//    for (int b=0; b<maxblocks; b++) {
//      printf("arg4.data[%d] = %e \n", b, ((double *)arg4.data)[b]);
//    }
//exit(-1);
    for (int b=0; b<maxblocks; b++)
      for (int d=0; d<1; d++) {
        arg4h[d] = arg4h[d] + ((double *)arg4.data)[d+b*1*64];
    //    printf("((double *)arg4.data)[d+b*1*64] = %e\n",((double *)arg4.data)[d+b*1*64]);
      }
