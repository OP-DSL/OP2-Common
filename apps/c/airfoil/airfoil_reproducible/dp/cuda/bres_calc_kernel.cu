...
__constant__ int opMap_pbecell_stride_temp_inc_OP2CONSTANT;
int opMap_pbecell_stride_temp_inc_OP2HOST;
//user function
__device__ void bres_calc_gpu( const double *x1, const double *x2, const double *q1,
  ...
  if (*bound == 1) {
    res1[(1)*opMap_pbecell_stride_temp_inc_OP2CONSTANT] += +p1 * dy;
    res1[(2)*opMap_pbecell_stride_temp_inc_OP2CONSTANT] += -p1 * dx;
  } else {
    ...
    res1[(0)*opMap_pbecell_stride_temp_inc_OP2CONSTANT] += f;
    ...
    res1[(1)*opMap_pbecell_stride_temp_inc_OP2CONSTANT] += f;
    ...
    res1[(2)*opMap_pbecell_stride_temp_inc_OP2CONSTANT] += f;
    ...
    res1[(3)*opMap_pbecell_stride_temp_inc_OP2CONSTANT] += f;
  }
...
__global__ void op_cuda_bres_calc(
  ...
  if (tid + start < end) {
    ...
    for ( int i=0; i< prime_map_dim * 4; i++ ){
      tmp_incs3_d[i+n*prime_map_dim * 4]=(double)0.0;
    }

    //user-supplied kernel call
    for ( int d=0; d<4; d++ ){
      tmp_incs3_d[map2idx+d*opMap_pbecell_stride_temp_inc_OP2CONSTANT]=(int)0.0;
    }
...
void op_par_loop_bres_calc(char const *name, op_set set,
  ...
  if (set->size > 0 && rev_map_pbecell != NULL ) {
    ...
    double *tmp_incs4_d = NULL;
    int required_tmp_incs_size4 = set_from_size_pbecell * prime_map_pbecell_dim * arg4.dat->size;
    reallocTempArrays(arg4.dat->index, required_tmp_incs_size4);
    tmp_incs4_d = (double *)op_repr_incs[arg4.dat->index].tmp_incs_d;
    ...
    if ((OP_kernels[3].count==1) || (opMap_pbecell_stride_temp_inc_OP2HOST != set_from_size_pbecell* prime_map_pbecell_dim)) {
      opMap_pbecell_stride_temp_inc_OP2HOST = set_from_size_pbecell* prime_map_pbecell_dim;
      cudaMemcpyToSymbol(opMap_pbecell_stride_temp_inc_OP2CONSTANT, &opMap_pbecell_stride_temp_inc_OP2HOST,sizeof(int));
    }
    ...
    for ( int round=0; round<2; round++ ){
      ...
      if (end-start>0) {
        ...
        arg2.map_data_d,
        (int*)arg5.data_d,
        start,end,set->size+set->exec_size,prime_map_pbecell_dim);
      }
    }
    int nblocks;

    nblocks = (set_to_size_pbecell-1)/nthread+1;
    apply_tmp_incs_soa<<<nblocks,nthread>>>(tmp_incs4_d, arg4.data_d, rev_map_pbecell->reversed_map_d, rev_map_pbecell->row_start_idx_d, arg4.dim, prime_map_pbecell_dim,set_to_size_pbecell,set_from_size_pbecell);
  }
