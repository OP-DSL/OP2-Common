...
mpirun -np 2 --oversubscribe ~/numawrap ./airfoil_mpi_cuda -i $T1 -o p_q_2.8m-${PREC1L}_it${T1}.h5 > $O1

if [[ "$PREC2" != "no" ]]; 
then
  cd ../$PREC2
  echo "Running in $PREC2L precision"
  mpirun -np 2 --oversubscribe ~/numawrap ./airfoil_mpi_cuda -i $T2 -d ../${PREC1}/p_q_2.8m-${PREC1L}_it${T1}.h5 -o p_q_2.8m-${T1}${PREC1L}_${T2}${PREC2L}.h5 > $O2
fi
