...
// user function
__device__ void adt_calc(const float *x1, const float *x2, const float *x3,
...
__global__ void op_cuda_adt_calc(const float *__restrict ind_arg0,
  ...
  for (int n = threadIdx.x; n < nelem; n += blockDim.x) {
    ...
    adt_calc(ind_arg0 + map0idx, ind_arg0 + map1idx, ind_arg0 + map2idx,
             ind_arg0 + map3idx, arg4 + (n + offset_b),
             arg5 + (n + offset_b) * 1);
  }
}
...
// get plan
  ...
  if (set->size > 0) {
    ...
    if ((OP_kernels[1].count == 1) ||
      ...
      opDat0_adt_calc_stride_OP2HOST = getSetSizeFromOpArg(&arg0);
      cudaMemcpyToSymbol(opDat0_adt_calc_stride_OP2CONSTANT,
                         &opDat0_adt_calc_stride_OP2HOST, sizeof(int));
    }
    if ((OP_kernels[1].count == 1) ||
        (direct_adt_calc_stride_OP2HOST != getSetSizeFromOpArg(&arg4))) {
      direct_adt_calc_stride_OP2HOST = getSetSizeFromOpArg(&arg4);
      cudaMemcpyToSymbol(direct_adt_calc_stride_OP2CONSTANT,
                         &direct_adt_calc_stride_OP2HOST, sizeof(int));
    }
    // execute plan
    ...
    for (int col = 0; col < Plan->ncolors; col++) {
      ...
      if (Plan->ncolblk[col] > 0) {
        op_cuda_adt_calc<<<nblocks, nthread>>>(
            (float *)arg0.data_d, arg0.map_data_d, (float *)arg4.data_d,
            (float *)arg5.data_d, block_offset, Plan->blkmap, Plan->offset,
