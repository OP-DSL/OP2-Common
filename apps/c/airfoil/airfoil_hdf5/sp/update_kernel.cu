...
// user function
__device__ void update(const float *qold, float *q, float *res,
                       const float *adt, float *rms) {
  float del, adti;
  float rmsl = 0.0f;
  adti = 1.0f / (*adt);
...
__global__ void op_cuda_update(const float *__restrict arg0, float *arg1,
                               float *arg2, const float *__restrict arg3,
                               float *arg4, int set_size) {
  ...
  for (int n = threadIdx.x + blockIdx.x * blockDim.x; n < set_size;
    ...
    update(arg0 + n, arg1 + n, arg2 + n, arg3 + n * 1, arg4_l);
  }
...
void op_par_loop_update(char const *name, op_set set, op_arg arg0, op_arg arg1,
  ...
  if (set->size > 0) {
    ...
    if ((OP_kernels[4].count == 1) ||
      ...
      direct_update_stride_OP2HOST = getSetSizeFromOpArg(&arg0);
      cudaMemcpyToSymbol(direct_update_stride_OP2CONSTANT,
                         &direct_update_stride_OP2HOST, sizeof(int));
    }
