...
__device__ void res_calc_gpu( const double *x1, const double *x2, const double *q1,
  ...
  p1 = gm1_hip * (q1[(3)*opDat2_res_calc_stride_OP2CONSTANT] - 0.5f * ri * (q1[(1)*opDat2_res_calc_stride_OP2CONSTANT] * q1[(1)*opDat2_res_calc_stride_OP2CONSTANT] + q1[(2)*opDat2_res_calc_stride_OP2CONSTANT] * q1[(2)*opDat2_res_calc_stride_OP2CONSTANT]));
  ...
  p2 = gm1_hip * (q2[(3)*opDat2_res_calc_stride_OP2CONSTANT] - 0.5f * ri * (q2[(1)*opDat2_res_calc_stride_OP2CONSTANT] * q2[(1)*opDat2_res_calc_stride_OP2CONSTANT] + q2[(2)*opDat2_res_calc_stride_OP2CONSTANT] * q2[(2)*opDat2_res_calc_stride_OP2CONSTANT]));
  vol2 = ri * (q2[(1)*opDat2_res_calc_stride_OP2CONSTANT] * dy - q2[(2)*opDat2_res_calc_stride_OP2CONSTANT] * dx);

  mu = 0.5f * ((*adt1) + (*adt2)) * eps_hip;
...
// HIP kernel function
__global__ void op_hip_res_calc(
  ...
  for ( int n=hipThreadIdx_x; n<ind_arg3_size; n+=hipBlockDim_x ){
    arg6_l[0] = ind_arg3_s[n+0*ind_arg3_size] + ind_arg3[ind_arg3_map[n]+0*opDat2_res_calc_stride_OP2CONSTANT];
    arg6_l[1] = ind_arg3_s[n+1*ind_arg3_size] + ind_arg3[ind_arg3_map[n]+1*opDat2_res_calc_stride_OP2CONSTANT];
    arg6_l[2] = ind_arg3_s[n+2*ind_arg3_size] + ind_arg3[ind_arg3_map[n]+2*opDat2_res_calc_stride_OP2CONSTANT];
    arg6_l[3] = ind_arg3_s[n+3*ind_arg3_size] + ind_arg3[ind_arg3_map[n]+3*opDat2_res_calc_stride_OP2CONSTANT];
    ind_arg3[ind_arg3_map[n]+0*opDat2_res_calc_stride_OP2CONSTANT] = arg6_l[0];
    ind_arg3[ind_arg3_map[n]+1*opDat2_res_calc_stride_OP2CONSTANT] = arg6_l[1];
    ind_arg3[ind_arg3_map[n]+2*opDat2_res_calc_stride_OP2CONSTANT] = arg6_l[2];
    ind_arg3[ind_arg3_map[n]+3*opDat2_res_calc_stride_OP2CONSTANT] = arg6_l[3];
  }
}
...
void op_par_loop_res_calc(char const *name, op_set set,
  ...
  if (set_size > 0) {
    ...
    if ((OP_kernels[2].count==1) || (opDat0_res_calc_stride_OP2HOST != getSetSizeFromOpArg(&arg0))) {
      ...
      hipMemcpyToSymbol(HIP_SYMBOL(opDat0_res_calc_stride_OP2CONSTANT), &opDat0_res_calc_stride_OP2HOST,sizeof(int));
    }
    if ((OP_kernels[2].count==1) || (opDat2_res_calc_stride_OP2HOST != getSetSizeFromOpArg(&arg2))) {
      ...
      hipMemcpyToSymbol(HIP_SYMBOL(opDat2_res_calc_stride_OP2CONSTANT), &opDat2_res_calc_stride_OP2HOST,sizeof(int));
    }
    //execute plan
    ...
    for ( int col=0; col<Plan->ncolors; col++ ){
      ...
      if (Plan->ncolblk[col] > 0) {
        ...
        hipLaunchKernelGGL(op_hip_res_calc,nblocks,nthread,nshared,0,
        (double *)arg0.data_d,
        (double *)arg2.data_d,
        (double *)arg4.data_d,
        (double *)arg6.data_d,
