...
__device__ void adt_calc_gpu( const double *x1, const double *x2, const double *x3,
  ...
  c = sqrt(gam_hip * gm1_hip * (ri * q[(3)*direct_adt_calc_stride_OP2CONSTANT] - 0.5f * (u * u + v * v)));
  ...
  *adt = (*adt) * (1.0f / cfl_hip);

}

// HIP kernel function
__global__ void op_hip_adt_calc(
  ...
  for ( int n=hipThreadIdx_x; n<nelem; n+=hipBlockDim_x ){
    int map0idx;
    int map1idx;
    int map2idx;
    int map3idx;
...
void op_par_loop_adt_calc(char const *name, op_set set,
  ...
  if (set_size > 0) {
    ...
    if ((OP_kernels[1].count==1) || (opDat0_adt_calc_stride_OP2HOST != getSetSizeFromOpArg(&arg0))) {
      ...
      hipMemcpyToSymbol(HIP_SYMBOL(opDat0_adt_calc_stride_OP2CONSTANT), &opDat0_adt_calc_stride_OP2HOST,sizeof(int));
    }
    if ((OP_kernels[1].count==1) || (direct_adt_calc_stride_OP2HOST != getSetSizeFromOpArg(&arg4))) {
      ...
      hipMemcpyToSymbol(HIP_SYMBOL(direct_adt_calc_stride_OP2CONSTANT),&direct_adt_calc_stride_OP2HOST,sizeof(int));
    }
    //execute plan
    ...
    for ( int col=0; col<Plan->ncolors; col++ ){
      ...
      if (Plan->ncolblk[col] > 0) {
        hipLaunchKernelGGL(op_hip_adt_calc,nblocks,nthread,0,0,
        (double *)arg0.data_d,
        arg0.map_data_d,
        (double*)arg4.data_d,
        (double*)arg5.data_d,
