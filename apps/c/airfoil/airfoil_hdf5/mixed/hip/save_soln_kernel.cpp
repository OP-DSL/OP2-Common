...
// HIP kernel function
__global__ void op_hip_save_soln(
...
void op_par_loop_save_soln(char const *name, op_set set,
  ...
  if (set_size > 0) {
    ...
    if ((OP_kernels[0].count==1) || (direct_save_soln_stride_OP2HOST != getSetSizeFromOpArg(&arg0))) {
      ...
      hipMemcpyToSymbol(HIP_SYMBOL(direct_save_soln_stride_OP2CONSTANT),&direct_save_soln_stride_OP2HOST,sizeof(int));
    }
    ...
    hipLaunchKernelGGL(op_hip_save_soln,nblocks,nthread,0,0,
      (double *) arg0.data_d,
      (double *) arg1.data_d,
      set->size );
  }
