    ...
    if ninds>0:
      ...
      for g_m in range(0, nargs):
        if maps[g_m] == OP_GBL:
          code('TYP ARG_l = *ARG;')
      # write kernel
      line = '#pragma omp target teams map(to:col_reord,'
      if nmaps > 0:
        k = []
        for g_m in range(0,nargs):
          if maps[g_m] == OP_MAP and (not invmapinds[inds[g_m]-1] in k):
      ...
      for g_m in range(0,nargs):
        ...
        if maps[g_m] == OP_ID:
          if soaflags[g_m]:
            ...
            line += '&data'+str(g_m)+'['+str(dims[g_m])+'*n]'
        if maps[g_m] == OP_MAP:
          if soaflags[g_m]:
...
#
      ...
      for g_m in range(0,nargs):
        if maps[g_m] == OP_ID:
          if soaflags[g_m]:
            line = line + indent + '&data'+str(g_m)+'[n_op]'
          else:
            line = line + indent + '&data'+str(g_m)+'['+str(dims[g_m])+'*n_op]'
        if maps[g_m] == OP_GBL:
          line = line + indent +'&arg'+str(g_m)+'_l'
        if g_m < nargs-1:
          line = line +','
...
##########################################################################
    fid = open(name+'_ompkernel.cpp','w')
...
##########################################################################
  ...
  for nk in range(0,len(kernels)):
    code('#include "'+kernels[nk]['name']+'_ompkernel.cpp"')
  master = master.split('.')[0]
  fid = open(master.split('.')[0]+'_ompkernels.cpp','w')
  fid.write('//\n// auto-generated by op2.py\n//\n\n')
  fid.write(file_text)
  fid.close()
