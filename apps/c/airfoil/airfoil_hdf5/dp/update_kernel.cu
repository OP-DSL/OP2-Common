...
__device__ void update( const double *qold, double *q, double *res, const double *adt, double *rms) {
  ...
  for (int n=0; n<4; n++) {
    del    = adti*res[n*direct_stride_OP2CONSTANT];
    q[n*direct_stride_OP2CONSTANT]   = qold[n*direct_stride_OP2CONSTANT] - del;
    res[n*direct_stride_OP2CONSTANT] = 0.0f;
    *rms  += del*del;
  }
}
...
void op_par_loop_update(char const *name, op_set set,
  ...
  if (set->size > 0) {
    ...
    if ((OP_kernels[4].count==1) || (direct_stride_OP2HOST.NE.getSetSizeFromOpArg(opArg1))) {
      direct_stride_OP2HOST = getSetSizeFromOpArg(opArg0);
