...
__device__ void bres_calc_gpu( const double *x1, const double *x2, const double *q1,
  ...
  if (*bound==1) {
    ...
    res1[0] += f;
    f = 0.5f*(vol1* q1[1*opDat2_bres_calc_stride_OP2CONSTANT] + p1*dy + vol2* qinf[1] + p2*dy) + mu*(q1[1*opDat2_bres_calc_stride_OP2CONSTANT]-qinf[1]);
    res1[1] += f;
    f = 0.5f*(vol1* q1[2*opDat2_bres_calc_stride_OP2CONSTANT] - p1*dx + vol2* qinf[2] - p2*dx) + mu*(q1[2*opDat2_bres_calc_stride_OP2CONSTANT]-qinf[2]);
    res1[2] += f;
    f = 0.5f*(vol1*(q1[3*opDat2_bres_calc_stride_OP2CONSTANT]+p1)     + vol2*(qinf[3]+p2)    ) + mu*(q1[3*opDat2_bres_calc_stride_OP2CONSTANT]-qinf[3]);
    res1[3] += f;
  }
}
