...
//user function
__device__ void res_calc_gpu( const long double *x1, const long double *x2, const long double *q1,
...
__global__ void op_cuda_res_calc(
  const long double *__restrict ind_arg0,
  const long double *__restrict ind_arg1,
  const long double *__restrict ind_arg2,
  long double *__restrict ind_arg3,
  ...
  if (tid + start < end) {
    ...
    long double arg6_l[4];
    for ( int d=0; d<4; d++ ){
      arg6_l[d] = ZERO_long double;
    }
    long double arg7_l[4];
    for ( int d=0; d<4; d++ ){
      arg7_l[d] = ZERO_long double;
    }
    int map0idx;
    int map1idx;
    int map2idx;
