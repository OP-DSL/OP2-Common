...
__device__ void res_calc_gpu( const double **x, const double **phim, double *K,
  ...
  for (int i = 0; i < 4; i++) {
    ...
    double a = 0;
    for (int m = 0; m < 4; m++)
      det_x_xi += Ng2_xi_cuda[4 * i + 16 + m] * x[m][(1)*opDat0_res_calc_stride_OP2CONSTANT];
    for (int m = 0; m < 4; m++)
      N_x[m] = det_x_xi * Ng2_xi_cuda[4 * i + m];

    a = 0;
    for (int m = 0; m < 4; m++)
      a += Ng2_xi_cuda[4 * i + m] * x[m][(0)*opDat0_res_calc_stride_OP2CONSTANT];
    for (int m = 0; m < 4; m++)
    ...
    a = 0;
    for (int m = 0; m < 4; m++)
      a += Ng2_xi_cuda[4 * i + m] * x[m][(1)*opDat0_res_calc_stride_OP2CONSTANT];
    for (int m = 0; m < 4; m++)
      N_x[m] -= a * Ng2_xi_cuda[4 * i + 16 + m];

    double b = 0;
    for (int m = 0; m < 4; m++)
      b += Ng2_xi_cuda[4 * i + 16 + m] * x[m][(0)*opDat0_res_calc_stride_OP2CONSTANT];
    for (int m = 0; m < 4; m++)
...
__global__ void op_cuda_res_calc(
  ...
  if (tid + start < end) {
    ...
    res_calc_gpu(arg0_vec,
             ...
             arg13_vec);
    if (optflags & 1<<2) {
      atomicAdd(&ind_arg3[0*opDat0_res_calc_stride_OP2CONSTANT+map0idx],arg13_l[0]);
      atomicAdd(&ind_arg3[1*opDat0_res_calc_stride_OP2CONSTANT+map0idx],arg13_l[1]);
    }
    if (optflags & 1<<2) {
      atomicAdd(&ind_arg3[0*opDat0_res_calc_stride_OP2CONSTANT+map1idx],arg14_l[0]);
      atomicAdd(&ind_arg3[1*opDat0_res_calc_stride_OP2CONSTANT+map1idx],arg14_l[1]);
    }
    if (optflags & 1<<2) {
      atomicAdd(&ind_arg3[0*opDat0_res_calc_stride_OP2CONSTANT+map2idx],arg15_l[0]);
      atomicAdd(&ind_arg3[1*opDat0_res_calc_stride_OP2CONSTANT+map2idx],arg15_l[1]);
    }
    if (optflags & 1<<2) {
      atomicAdd(&ind_arg3[0*opDat0_res_calc_stride_OP2CONSTANT+map3idx],arg16_l[0]);
      atomicAdd(&ind_arg3[1*opDat0_res_calc_stride_OP2CONSTANT+map3idx],arg16_l[1]);
    }
  }
}
