...
__device__ void update( const float *r, float *du, float *u, float *u_sum, float *u_max) {
  ...
  u_sum[0*direct_update_stride_OP2CONSTANT] += (u[0*direct_update_stride_OP2CONSTANT])*(u[0*direct_update_stride_OP2CONSTANT]);
  u_max[0*direct_update_stride_OP2CONSTANT] = MAX(u[0*direct_update_stride_OP2CONSTANT],u[0*direct_update_stride_OP2CONSTANT]);
}
