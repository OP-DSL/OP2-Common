...
__global__ void op_cuda_update(
  ...
  for (int n=threadIdx.x+blockIdx.x*blockDim.x;
    ...
    for (int m=0; m<2; m++)
      ((double *)arg_s)[tid+m*nelems] = arg0[tid+m*nelems+offset*2];

    for (int m=0; m<2; m++)
      arg0_l[m] = ((double *)arg_s)[m+tid*2];

    for (int m=0; m<3; m++)
      ((double *)arg_s)[tid+m*nelems] = arg1[tid+m*nelems+offset*3];

    for (int m=0; m<3; m++)
      arg1_l[m] = ((double *)arg_s)[m+tid*3];

    for (int m=0; m<2; m++)
      ((double *)arg_s)[tid+m*nelems] = arg2[tid+m*nelems+offset*2];

    for (int m=0; m<2; m++)
      arg2_l[m] = ((double *)arg_s)[m+tid*2];
    ...
    for (int m=0; m<3; m++)
      ((double *)arg_s)[m+tid*3] = arg1_l[m];

    for (int m=0; m<3; m++)
      arg1[tid+m*nelems+offset*3] = ((double *)arg_s)[tid+m*nelems];

    for (int m=0; m<2; m++)
      ((double *)arg_s)[m+tid*2] = arg2_l[m];

    for (int m=0; m<2; m++)
      arg2[tid+m*nelems+offset*2] = ((double *)arg_s)[tid+m*nelems];
