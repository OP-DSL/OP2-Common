...
// user function
__device__ void res_gpu(const double *A, const float *u, float *du,
                        const float *beta) {
  *du += (float)((*beta) * (*A) * (*u));
}
...
__global__ void
op_cuda_res(const float *__restrict ind_arg0, float *__restrict ind_arg1,
            const int *__restrict opDat1Map, const double *__restrict arg0,
            const float *arg3, int *ind_map, short *arg_map, int *ind_arg_sizes,
  ...
  for (int n = threadIdx.x; n < nelems2; n += blockDim.x) {
    ...
    if (n < nelem) {
      ...
      res_gpu(arg0 + (n + offset_b) * 3, ind_arg0 + map1idx * 2, arg2_l, arg3);
    ...
    for (int col = 0; col < Plan->ncolors; col++) {
      ...
      if (Plan->ncolblk[col] > 0) {
        ...
        op_cuda_res<<<nblocks, nthread, nshared>>>(
            (float *)arg1.data_d, (float *)arg2.data_d, arg1.map_data_d,
            (double *)arg0.data_d, (float *)arg3.data_d, Plan->ind_map,
            Plan->loc_map, Plan->ind_sizes, Plan->ind_offs, block_offset,
            Plan->blkmap, Plan->offset, Plan->nelems, Plan->nthrcol,
            Plan->thrcol, Plan->ncolblk[col], set->size + set->exec_size);
      }
