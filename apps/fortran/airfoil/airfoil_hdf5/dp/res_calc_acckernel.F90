...
SUBROUTINE op_wrap_res_calc( &
  ...
  !$acc routine(res_calc)


  !$acc parallel loop independent gang vector deviceptr( opDat1Local, opDat3Local, opDat5Local, opDat7Local, opDat1Map, opDat3Map, col_reord) private(i1, map1idx, map2idx, map3idx, map4idx)
...
SUBROUTINE res_calc_host( userSubroutine, set, &
  ...
  DO i1 = 0, actualPlan_res_calc%ncolors-1, 1
    ...
    IF (i1 .EQ. actualPlan_res_calc%ncolors_owned -1) THEN
    END IF
  END DO
