...
! End interface of call-back routine
  ...
  procedure(called_proc), bind(c), pointer :: proc_update

  IF (.not. JIT_COMPILED) THEN
    call jit_compile()
  END IF
  call c_f_procpointer( proc_addr_update, proc_update )

! call/execute dynamically loaded procedure with the parameters from update_host() signature
  call proc_update( userSubroutine, set, &
  & opArg1, &
  & opArg2, &
  & opArg3, &
  & opArg4, &
