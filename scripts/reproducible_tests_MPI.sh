...
do
    ...
    echo ""
    echo "norepro"
    arr_airfoil_nonrepro+=($(mpirun -mca io ^ompio -np $NP ~/numawrap ./airfoil_mpi_genseq_norepro 2>&1 | tee -a $norepfile | grep -oP "Max total runtime = \K.*"))
    echo "temparray"
    arr_airfoil_temparray+=($(mpirun -mca io ^ompio -np $NP ~/numawrap ./airfoil_mpi_genseq_temparray 2>&1 | tee -a $temparrayfile | grep -oP "Max total runtime = \K.*"))
    echo "greedycol"
    arr_airfoil_greedycol+=($(mpirun -mca io ^ompio -np $NP ~/numawrap ./airfoil_mpi_genseq_reprcolor -op_repro_greedy_coloring 2>&1 | tee -a $greedycolfile | grep -oP "Max total runtime = \K.*"))
    echo "distcol"
    arr_airfoil_distrcol+=($(mpirun -mca io ^ompio -np $NP ~/numawrap ./airfoil_mpi_genseq_reprcolor 2>&1 | tee -a $distfile | grep -oP "Max total runtime = \K.*"))
    ...
    echo ""
    echo "norepro"
    arr_aero_nonrepro+=($(mpirun -mca io ^ompio -np $NP ~/numawrap ./aero_mpi_genseq_norepro 2>&1 | tee -a $norepfile | grep -oP "Max total runtime = \K.*"))
    echo "temparray"
    arr_aero_temparray+=($(mpirun -mca io ^ompio -np $NP ~/numawrap ./aero_mpi_genseq_temparray 2>&1 | tee -a $temparrayfile | grep -oP "Max total runtime = \K.*"))
    echo "greedycol"
    arr_aero_greedycol+=($(mpirun -mca io ^ompio -np $NP ~/numawrap ./aero_mpi_genseq_reprcolor -op_repro_greedy_coloring 2>&1 | tee -a $greedycolfile | grep -oP "Max total runtime = \K.*"))
    echo "distcol"
    arr_aero_distrcol+=($(mpirun -mca io ^ompio -np $NP ~/numawrap ./aero_mpi_genseq_reprcolor 2>&1 | tee -a $distfile | grep -oP "Max total runtime = \K.*"))
    ...
    echo " "
    echo "norepro"
    arr_mgcfd_nonrepro+=($(mpirun -mca io ^ompio -np $NP ~/numawrap  ../MG-CFD-app-OP2/bin/mgcfd_mpi_norepro -i input-mgcfd.dat -m ptscotch OP_PART_SIZE=2048 2>&1 | tee -a $norepfile | grep -oP "Max total runtime = \K.*"))
    echo "temparray"
    arr_mgcfd_temparray+=($(mpirun -mca io ^ompio -np $NP ~/numawrap  ../MG-CFD-app-OP2/bin/mgcfd_mpi_temparray -i input-mgcfd.dat -m ptscotch OP_PART_SIZE=2048 2>&1 | tee -a $temparrayfile | grep -oP "Max total runtime = \K.*"))
    echo "greedycol"
    arr_mgcfd_greedycol+=($(mpirun -mca io ^ompio -np $NP ~/numawrap  ../MG-CFD-app-OP2/bin/mgcfd_mpi_genseq_reprcolor -i input-mgcfd.dat -m ptscotch OP_PART_SIZE=2048 -op_repro_greedy_coloring 2>&1 | tee -a $greedycolfile | grep -oP "Max total runtime = \K.*"))
    echo "distcol"
    arr_mgcfd_distrcol+=($(mpirun -mca io ^ompio -np $NP ~/numawrap  ../MG-CFD-app-OP2/bin/mgcfd_mpi_genseq_reprcolor -i input-mgcfd.dat -m ptscotch OP_PART_SIZE=2048 2>&1 | tee -a $distfile | grep -oP "Max total runtime = \K.*"))
